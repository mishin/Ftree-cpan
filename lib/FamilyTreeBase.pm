...
use version; our $VERSION = qv('2.3.13');
...
sub _toppage {
  ...
  print $self->{cgi}->header( -charset => 'UTF-8' ),
     $self->{cgi}->start_html(-title=>$title,
...
sub _endpage {
  ...
  print "\n",'<script src="http://www.google-analytics.com/urchin.js" type="text/javascript">',
      ...
      'urchinTracker();',
      '</script>', "\n";
  print $self->{cgi}->start_i, $self->{textGenerator}->maintainer( $self->{settings}{adminName},
    $self->{settings}{adminEmail},  $self->{settings}{adminHomepage} ), $self->{cgi}->br,
    $self->{textGenerator}->software($VERSION), "\n",
    $self->{cgi}->end_i, $self->{cgi}->br;
  print $self->{cgi}->i($self->{textGenerator}{DonationSentence}),<<"END_PAYPAL";
  ...
  sub language_chooser {
    ...
    while ( my ($lang, $pic) = each %lang_to_pict ) {
      print $self->{cgi}->td({-align => 'center'},
        $self->{cgi}->a({-href => "${anchor}lang=$pic",
               -title => $self->{textGenerator}->{$lang} },
          $self->{cgi}->img({-width => 40, -src =>"$self->{graphicsUrl}/flags/${pic}.gif",
               -alt =>$self->{textGenerator}->{$lang}}))),"\n",
    }
    print $self->{cgi}->end_Tr, "\n", $self->{cgi}->end_table, $self->{cgi}->br,"\n";
  	...
  	my $picture_file = defined $person->get_default_picture() ?
  		                 ...
  		                 $self->{graphicsUrl} . ( 
  		                   defined $person->get_gender() ?
                               $person->get_gender() == 0 ?  '/nophoto_m.jpg' : '/nophoto_f.jpg' :  
                               '/nophoto.gif' ); 
